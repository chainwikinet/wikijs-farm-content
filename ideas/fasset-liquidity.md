...
The fAsset reward pools could be replaced with fAsset vaults. These vaults could implement a strategy that automatically market makes trades between an fAsset and its underlying asset to create inexpensive liquidity for them.

The strategy could batch deposit/withdraw the underlying to maintain inventory of both for trading. It could make a small spread on trades. Harvest would only need to pay enough to cover forgone interest on the portion of fAsset withdrawn to the underlying to maintain inventory, a few percent per year.

There is a 0x contract type that can accept deposits and continuously write limit orders against them using any onchain data to set the limit price. https://github.com/0xProject/0x-monorepo/commit/68207aac80103c3af7aa3912e674a7257208ec81
